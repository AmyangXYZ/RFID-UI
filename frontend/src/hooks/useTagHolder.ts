...
export function useTagHolder() {
  ...
  const registerTag = function (id: string) {
    ...
    // useFetch("http://192.168.0.100:16311/api/ui/tag/"+id).post().text()//UI to server
      .post()
      .text()
  ...
  const deleteTag = function (id24: string) {
    ...
    // useFetch("http://192.168.0.100:16311/api/ui/tagdelete/"+id24).post().text()
      .post()
      .text()
    setTimeout(getAllTags, 250)
  }
  ...
  const enterDistance = function(UIdist: string){
    ...
    // useFetch("http://192.168.0.100:16311/api/ui/distance/"+UIdist).post().text()//UI to server
